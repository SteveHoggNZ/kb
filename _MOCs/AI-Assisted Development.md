...
### [[concepts/ai-development/Four-Tier Memory Model|Four-Tier Memory Model]]
Agent "memory" isn't one thing but four tiers, each with a different store/retrieval/failure profile: **Working** (in-thread, ephemeral — solved without a store via [[concepts/ai-development/Contextual Breadcrumbs|Contextual Breadcrumbs]]), **Episodic** (what happened — a query/filter problem, not embeddings), **Semantic** (durable facts + the invalidation/"superseded at a time" problem — [[concepts/ai-development/Ontologies as Agent Guardrails|ontologies]]), and **Procedural** (how work gets done, learned from correction — [[concepts/ai-development/Writing Great Skills|Agent Skills]]; highest ROI, cheapest to build because the corrections are already in your transcripts). Stops teams reaching for a vector DB when they actually need a query, a constraint, or a skill.
